...
Item {
  ...
  readonly property var inputBorderSpec: errorState
    ...
    : Border.surfaceSpec("lock", "text", Color.lock.text, root.outlineThickness, "border-alpha")
  ...
  function forcePasswordFocus() {
    if (passwordInput.enabled && !passwordInput.activeFocus)
  ...
  Rectangle {
    ...
    Item {
      // Keep the clock group stable while the password field sits slightly
      // below center.
      ...
      Text {
        ...
        width: parent.width
        height: 42
        text: Qt.formatDate(lockClock.date, "dddd, MMMM d")
        color: Color.lock.text
        opacity: 0.8
        horizontalAlignment: Text.AlignHCenter
        verticalAlignment: Text.AlignVCenter
    ...
    BorderSurface {
      ...
      TextInput {
        ...
        onEnabledChanged: {
          if (enabled) Qt.callLater(root.forcePasswordFocus)
        }
        onActiveFocusChanged: {
          if (!activeFocus && enabled) focusRecoveryTimer.restart()
        }
